...
| `ogImage` | Optional | A custom **share preview** image (the picture shown when you paste the link on WhatsApp/X/etc.). Make it **1200×630** and point to a file in the post's `public/posts/...` folder. **Shortcut:** if you instead just name the file `share.png`/`og.png` and drop it in `public/posts/<post-name>/`, it's picked up automatically with no front-matter (this field still overrides it). If you skip both, a card is generated for you. See [Covers & images](./covers-and-images.md#share-previews-when-you-post-a-link). |
